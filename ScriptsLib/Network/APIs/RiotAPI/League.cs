...
namespace ScriptsLib.Network.APIs
{
	public static partial class RiotAPI
	{
		public static class League // v4
		{
			...
			public static string GetChallengerEntries(Queues queue) => GET(ServerString() + $"/lol/league/v4/challengerleagues/by-queue/{queue}" + ApiString());

			/// <summary>Get league entries in all queues for a given summoner ID.</summary>
			public static string GetSummonerLeagues(string encryptedSummonerId) => GET(ServerString() + $"/lol/league/v4/entries/by-summoner/{encryptedSummonerId}" + ApiString());
			...
			public static string GetGrandmasterEntries(Queues queue) => GET(ServerString() + $"/lol/league/v4/grandmasterleagues/by-queue/{queue}" + ApiString());

			/// <summary>Get league with given ID, including inactive entries.</summary>
			/// <param name="leagueId">The UUID of the league.</param>
			public static string GetLeagueById(int leagueId) => GET(ServerString() + $"/lol/league/v4/leagues/{leagueId}" + ApiString());

			/// <summary>Get the master league for given queue.</summary>
			public static string GetMasterEntries(Queues queue) => GET(ServerString() + $"/lol/league/v4/masterleagues/by-queue/{queue}" + ApiString());
		}
	}
}
